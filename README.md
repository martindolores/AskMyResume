...
### Milestone 2 writeup: real AKS excursion

Ran on 2026-08-07. A single-node AKS cluster (`Standard_D2s_v7`, Free control-plane tier — the subscription's SKU allow-list didn't include the cheaper B-series) was created in a dedicated resource group, the exact manifests from `k8s/` were applied unmodified, the pod came up `1/1 Running`, and a port-forwarded `POST /chat` call returned a real RAG answer sourced from the resume text and generated by Gemini. The resource group was deleted immediately after capturing evidence — total cluster lifetime was about 15 minutes, entirely inside Azure free trial credit. Full `kubectl` output and the live `/chat` response are in [`docs/aks-evidence/pr-11-output.md`](docs/aks-evidence/pr-11-output.md).
